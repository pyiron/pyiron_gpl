...
def write_yopt(input_dict, working_directory="."):
    body = common.format(
        rcut=input_dict["rcut"] / angstrom,
        alpha_scale=input_dict["alpha_scale"],
        gcut_scale=input_dict["gcut_scale"],
        smooth_ei=input_dict["smooth_ei"],
        h5step=1,
    )
...
def write_yopt_cell(input_dict, working_directory="."):
    ...
    body += "dof = StrainCellDOF(ff, gpos_rms={gpos_rms}, dpos_rms={dpos_rms}, grvecs_rms={grvecs_rms}, drvecs_rms={drvecs_rms}, do_frozen=False)".format(
        gpos_rms=input_dict["gpos_rms"],
        dpos_rms=input_dict["dpos_rms"],
        grvecs_rms=input_dict["grvecs_rms"],
        drvecs_rms=input_dict["drvecs_rms"],
    )
    body += """
opt = CGOptimizer(dof, hooks=[hdf5])
opt.run({nsteps})
...
f['system/energy'] = energy
f['system/gpos'] = gpos
f['system/hessian'] = hessian""".format(
        hessian_eps=input_dict["hessian_eps"]
    )
    body += tail
    with open(posixpath.join(working_directory, "yscript.py"), "w") as f:
        f.write(body)
...
md.run({nsteps})
""".format(
        timestep=input_dict["timestep"] / femtosecond, nsteps=input_dict["nsteps"]
    )
    body += tail
    with open(posixpath.join(working_directory, "yscript.py"), "w") as f:
        f.write(body)
...
md.run({nsteps})
""".format(
        temp=input_dict["temp"] / kelvin,
        timestep=input_dict["timestep"] / femtosecond,
        timecon_thermo=input_dict["timecon_thermo"] / femtosecond,
        nsteps=input_dict["nsteps"],
    )
    body += tail
    with open(posixpath.join(working_directory, "yscript.py"), "w") as f:
...
""".format(
        temp=input_dict["temp"] / kelvin,
        timestep=input_dict["timestep"] / femtosecond,
        press=input_dict["press"] / bar,
        timecon_thermo=input_dict["timecon_thermo"] / femtosecond,
        timecon_baro=input_dict["timecon_baro"] / femtosecond,
        nsteps=input_dict["nsteps"],
    )
    body += tail
    with open(posixpath.join(working_directory, "yscript.py"), "w") as f:
...
def write_plumed_enhanced(input_dict, working_directory="."):
    ...
    with open(posixpath.join(working_directory, "plumed.dat"), "w") as f:
            ...
            if len(enhanced["icindices"][i] > 0):
                f.write(
                    ...
                    % (
                        i,
                        kind.upper(),
                        ",".join([str(icidx) for icidx in enhanced["icindices"][i]]),
                    )
                )
            else:
                f.write("ic%i: %s \n" % (i, kind.upper()))
        ...
        if "sigma" in enhanced.keys():
            ...
            f.write(
                "metad: METAD ARG=%s SIGMA=%s HEIGHT=%s PACE=%i FILE=%s \n"
                % (
            ...
            f.write(
                "PRINT ARG=%s,metad.bias FILE=%s STRIDE=%i \n"
                % (
                    ",".join(["ic%i" % i for i in range(len(enhanced["ickinds"]))]),
                    enhanced["file_colvar"],
                    enhanced["stride"],
                )
            )
        ...
        if "kappa" in enhanced.keys():
            ...
            f.write(
                "umbrella: RESTRAINT ARG=%s KAPPA=%s AT=%s \n"
                % (
                    ",".join(["ic%i" % i for i in range(len(enhanced["ickinds"]))]),
                    kappa,
                    loc,
            ...
            # setup printing of colvar
            f.write(
                "PRINT ARG=%s,umbrella.bias FILE=%s STRIDE=%i \n"
                % (
                    ",".join(["ic%i" % i for i in range(len(enhanced["ickinds"]))]),
                    enhanced["file_colvar"],
                    enhanced["stride"],
...
class Yaff(AtomisticGenericJob):
    ...
    def set_mtd(
        ...
        **Arguments**
        ...
        icindices = np.array(
            [np.array(ic[1]) + 1 for ic in ics]
        )  # plumed starts counting from 1
    ...
    def write_input(self):
        ...
        if self.input["jobtype"] == "sp":
            ...
            write_yopt_cell(
                input_dict=input_dict, working_directory=self.working_directory
            )
            ...
            raise IOError(
                "Invalid job type for Yaff job, received %s" % self.input["jobtype"]
            )
    ...
    def from_hdf(self, hdf=None, group_name=None):
        ...
        with self.project_hdf5.open("input") as hdf5_input:
            ...
            self.ffatypes = np.char.decode(
                hdf5_input["generic/ffatypes"]
            )  # decode byte string literals
    ...
    def get_yaff_system(self, snapshot=0):
        ...
        if cell is None:
            system = System(
                numbers, pos, ffatypes=self.ffatypes, ffatype_ids=self.ffatype_ids
            )
        else:
            system = System(
                numbers,
    ...
    def mtd_sum_hills_1d(self, fn=None):
        ...
        **Arguments**
        ...
        subprocess.check_output(
            "ml load PLUMED/2.5.2-intel-2019a-Python-3.7.2; plumed sum_hills --hills {} --outfile {}".format(
                fn, fn_out
            ),
