...
class Gaussian(GenericDFTJob):
    ...
    def collect_output(self):
        output_dict = collect_output(
            output_file=os.path.join(self.working_directory, "input.fchk")
        )
    ...
    def calc_minimize(
        self, electronic_steps=None, ionic_steps=None, algorithm=None, ionic_forces=None
    ):
        """
        ...
        **Arguments**
        ...
        self.input["jobtype"] = "opt" + "({})".format(",".join(opt_settings)) * (
            len(opt_settings) > 0
        )
    ...
    def calc_static(self, electronic_steps=None, algorithm=None):
        ...
        **Arguments**
        ...
        super(Gaussian, self).calc_static(
            electronic_steps=electronic_steps, algorithm=algorithm
        )

    def calc_md(
        self, temperature=None, n_ionic_steps=1000, time_step=None, n_print=100
    ):
    ...
    def print_MO(self):
        ...
        for n, index in enumerate(range(n_MO)):
            ...
            if self.get("output/structure/dft/beta_orbital_e") is None:
                ...
                print(
                    "#{}: \t Orbital energy = {:>10.5f} \t Occ. = {}".format(
                        n, orbital_energy, occ_alpha + occ_beta
                    )
                )
            else:
                orbital_energy = [
                ...
                print(
                    "#{}: \t Orbital energies (alpha,beta) = {:>10.5f},{:>10.5f} \t Occ. = {},{}".format(
                        n, orbital_energy[0], orbital_energy[1], occ_alpha, occ_beta
                    )
                )
    ...
    def visualize_MO(self, index, particle_size=0.5, show_bonds=True):
        ...
        **Notes**
        ...
        assert (
            len(self.get("output/structure/numbers")) < 50
        )  # check whether structure does not become too large for interactive calculation of cube file
        ...
        if self.get("output/structure/dft/beta_orbital_e") is None:
            ...
            print(
                "Orbital energies (alpha,beta) = {:>10.5f},{:>10.5f} \t Occ. = {},{}".format(
                    orbital_energy[0], orbital_energy[1], occ_alpha, occ_beta
                )
            )
        ...
        out = subprocess.check_output(
            "ml load Gaussian/g16_E.01-intel-2019a;module use /apps/gent/CO7/haswell-ib/modules/all; cubegen 1 MO={} {}.fchk {}.cube".format(
                index + 1, path, path
            ),
    ...
    def bsse_to_pandas(self):
        ...
        """
        assert "counterpoise" in [
            k.lower() for k in self.input["settings"].keys()
        ]  # check if there was a bsse calculation
        tmp = {}
...
class GaussianInput(GenericParameters):
    def __init__(self, input_file_name=None):
        super(GaussianInput, self).__init__(
...
def write_input(input_dict, working_directory="."):
    ...
    if not input_dict["mem"] is None:
        mem = input_dict["mem"] + "B" * (
            input_dict["mem"][-1] != "B"
        )  # check if string ends in bytes
        # convert pmem to mem
    ...
    if "Counterpoise" in settings.keys():
        if input_dict["bsse_idx"] is None or not len(input_dict["bsse_idx"]) == len(
            pos
        ):  # check if all elements are present for a BSSE calculation
            raise ValueError("The Counterpoise setting requires a valid bsse_idx array")
        # Check bsse idx (should start from 1 for Gaussian)
        input_dict["bsse_idx"] = [
            k - min(input_dict["bsse_idx"]) + 1 for k in input_dict["bsse_idx"]
        ]
        # Check if it only contains conseqcutive numbers (sum of set should be n*(n+1)/2)
        assert (
            sum(set(input_dict["bsse_idx"]))
            == (max(input_dict["bsse_idx"]) * (max(input_dict["bsse_idx"]) + 1)) / 2
        )
    ...
    # Write to file
    route_section = "#{} {}/{} {} {}\n\n".format(
        verbosity, lot, basis_set, jobtype, settings_string
    )
    with open(os.path.join(working_directory, "input.com"), "w") as f:
        ...
        if not "Counterpoise" in settings.keys():
            ...
            for n, p in enumerate(pos):
                f.write(
                    " {}\t{: 1.6f}\t{: 1.6f}\t{: 1.6f}\n".format(
                        symbols[n], p[0], p[1], p[2]
                    )
                )
            f.write("\n\n")  # don't know whether this is still necessary in G16
        else:
            if isinstance(charge, list) and isinstance(
                spin_mult, list
            ):  # for BSSE it is possible to define charge and multiplicity for the fragments separately
                f.write(
                    " ".join(
                        [
                            "{},{}".format(charge[idx], spin_mult[idx])
                            for idx in range(int(settings["Counterpoise"]))
                        ]
                    )
                )  # first couple is for full system, then every fragment separately
            else:
                f.write("{} {}\n".format(charge, spin_mult))

            for n, p in enumerate(pos):
                f.write(
                    " {}(Fragment={})\t{: 1.6f}\t{: 1.6f}\t{: 1.6f}\n".format(
                        symbols[n], input_dict["bsse_idx"][n], p[0], p[1], p[2]
                    )
                )
            f.write("\n\n")  # don't know whether this is still necessary in G16
...
def fchk2dict(fchk):
    ...
    fchkdict["structure/dft/n_alpha_electrons"] = fchk.fields.get(
        "Number of alpha electrons"
    )
    fchkdict["structure/dft/n_beta_electrons"] = fchk.fields.get(
        "Number of beta electrons"
    )
    fchkdict["structure/dft/n_basis_functions"] = fchk.fields.get(
        "Number of basis functions"
    )

    # Orbital information
    fchkdict["structure/dft/alpha_orbital_e"] = fchk.fields.get(
        "Alpha Orbital Energies"
    )
    fchkdict["structure/dft/beta_orbital_e"] = fchk.fields.get("Beta Orbital Energies")

    # Densities
    fchkdict["structure/dft/scf_density"] = _triangle_to_dense(
        fchk.fields.get("Total SCF Density")
    )
    fchkdict["structure/dft/spin_scf_density"] = _triangle_to_dense(
        fchk.fields.get("Spin SCF Density")
    )

    if fchk.lot.upper() in ["MP2", "MP3", "CC", "CI"]:
        # only one of the lots should be present, hence using the same key
        fchkdict["structure/dft/post_scf_density"] = _triangle_to_dense(
            fchk.fields.get("Total {} Density".format(fchk.lot))
        )
        fchkdict["structure/dft/post_spin_scf_density"] = _triangle_to_dense(
            fchk.fields.get("Spin {} Density".format(fchk.lot))
        )

    # Specific job information
    if fchkdict["jobtype"] == "fopt":
        if len(fchk.get_optimization_coordinates().shape) == 3:
            fchkdict["structure/positions"] = (
                fchk.get_optimization_coordinates()[-1] / angstrom
            )
        else:
            fchkdict["structure/positions"] = (
                fchk.get_optimization_coordinates() / angstrom
            )
        ...
        fchkdict["generic/forces"] = (
            fchk.get_optimization_gradients() / (electronvolt / angstrom) * -1
        )

    if fchkdict["jobtype"] == "freq":
        fchkdict["structure/positions"] = (
            fchk.fields.get("Current cartesian coordinates").reshape([1, -1, 3])
            / angstrom
        )
        fchkdict["generic/positions"] = (
            fchk.fields.get("Current cartesian coordinates").reshape([1, -1, 3])
            / angstrom
        )
        fchkdict["generic/forces"] = (
            fchk.fields.get("Cartesian Gradient").reshape([-1, 3])
            / (electronvolt / angstrom)
            * -1
        )
        fchkdict["generic/hessian"] = fchk.get_hessian() / (
...
def read_bsse(output_file, output_dict):
    ...
    if cp:
        ...
        frames = (
            1
            if isinstance(output_dict["generic/energy_tot"], float)
            else len(output_dict["generic/energy_tot"])
        )
        ...
        for i in range(frames):
            ...
            while not found:
                ...
                if "complexation energy" in line:
                    ...
                    output_dict["structure/bsse/complexation_energy_corrected"][
                        i
                    ] = cE_corr
                    found = True

        if frames == 1:
            output_dict["structure/bsse/energy_tot_corrected"] = output_dict[
                "structure/bsse/energy_tot_corrected"
            ][0]
            output_dict["structure/bsse/bsse_correction"] = output_dict[
                "structure/bsse/bsse_correction"
            ][0]
            output_dict["structure/bsse/sum_of_fragments"] = output_dict[
                "structure/bsse/sum_of_fragments"
            ][0]
            output_dict["structure/bsse/complexation_energy_raw"] = output_dict[
                "structure/bsse/complexation_energy_raw"
            ][0]
            output_dict["structure/bsse/complexation_energy_corrected"] = output_dict[
                "structure/bsse/complexation_energy_corrected"
            ][0]
        else:
            # flip array sequence
            output_dict["structure/bsse/energy_tot_corrected"] = output_dict[
...
def read_EmpiricalDispersion(output_file, output_dict):
    ...
    while True:
        ...
        if search_term in line:
            disp = (
                float(line[38:-9]) / electronvolt
            )  # could be changed when new search terms are implemented
            break
